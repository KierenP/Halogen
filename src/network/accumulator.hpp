...
template <size_t SIZE>
void add2sub4(std::array<int16_t, SIZE>& a, const std::array<int16_t, SIZE>& b, const std::array<int16_t, SIZE>& c,
    const std::array<int16_t, SIZE>& d, const std::array<int16_t, SIZE>& e, const std::array<int16_t, SIZE>& f,
    const std::array<int16_t, SIZE>& g, const std::array<int16_t, SIZE>& h)
{
#if defined(SIMD_ENABLED)
    constexpr auto stride = SIMD::vec_size / sizeof(int16_t);
    static_assert(SIZE % (stride * 4) == 0);
    for (size_t i = 0; i < SIZE; i += stride * 4)
    {
        ...
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&f[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&f[i + stride * 3]));
        b_vec1 = SIMD::sub_i16(b_vec1, SIMD::load(&g[i]));
        b_vec2 = SIMD::sub_i16(b_vec2, SIMD::load(&g[i + stride]));
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&g[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&g[i + stride * 3]));
        b_vec1 = SIMD::sub_i16(b_vec1, SIMD::load(&h[i]));
        b_vec2 = SIMD::sub_i16(b_vec2, SIMD::load(&h[i + stride]));
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&h[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&h[i + stride * 3]));
        SIMD::store(&a[i], b_vec1);
        SIMD::store(&a[i + stride], b_vec2);
        SIMD::store(&a[i + stride * 2], b_vec3);
        SIMD::store(&a[i + stride * 3], b_vec4);
...
template <size_t SIZE>
void add4sub4(std::array<int16_t, SIZE>& a, const std::array<int16_t, SIZE>& b, const std::array<int16_t, SIZE>& c,
    const std::array<int16_t, SIZE>& d, const std::array<int16_t, SIZE>& e, const std::array<int16_t, SIZE>& f,
    const std::array<int16_t, SIZE>& g, const std::array<int16_t, SIZE>& h, const std::array<int16_t, SIZE>& p,
    const std::array<int16_t, SIZE>& q)
{
#if defined(SIMD_ENABLED)
    constexpr auto stride = SIMD::vec_size / sizeof(int16_t);
    static_assert(SIZE % (stride * 4) == 0);
    for (size_t i = 0; i < SIZE; i += stride * 4)
    {
        ...
        b_vec3 = SIMD::add_i16(b_vec3, SIMD::load(&d[i + stride * 2]));
        b_vec4 = SIMD::add_i16(b_vec4, SIMD::load(&d[i + stride * 3]));
        b_vec1 = SIMD::add_i16(b_vec1, SIMD::load(&e[i]));
        b_vec2 = SIMD::add_i16(b_vec2, SIMD::load(&e[i + stride]));
        b_vec3 = SIMD::add_i16(b_vec3, SIMD::load(&e[i + stride * 2]));
        b_vec4 = SIMD::add_i16(b_vec4, SIMD::load(&e[i + stride * 3]));
        b_vec1 = SIMD::add_i16(b_vec1, SIMD::load(&f[i]));
        b_vec2 = SIMD::add_i16(b_vec2, SIMD::load(&f[i + stride]));
        b_vec3 = SIMD::add_i16(b_vec3, SIMD::load(&f[i + stride * 2]));
        b_vec4 = SIMD::add_i16(b_vec4, SIMD::load(&f[i + stride * 3]));
        b_vec1 = SIMD::sub_i16(b_vec1, SIMD::load(&g[i]));
        b_vec2 = SIMD::sub_i16(b_vec2, SIMD::load(&g[i + stride]));
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&g[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&g[i + stride * 3]));
        ...
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&h[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&h[i + stride * 3]));
        b_vec1 = SIMD::sub_i16(b_vec1, SIMD::load(&p[i]));
        b_vec2 = SIMD::sub_i16(b_vec2, SIMD::load(&p[i + stride]));
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&p[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&p[i + stride * 3]));
        b_vec1 = SIMD::sub_i16(b_vec1, SIMD::load(&q[i]));
        b_vec2 = SIMD::sub_i16(b_vec2, SIMD::load(&q[i + stride]));
        b_vec3 = SIMD::sub_i16(b_vec3, SIMD::load(&q[i + stride * 2]));
        b_vec4 = SIMD::sub_i16(b_vec4, SIMD::load(&q[i + stride * 3]));
        SIMD::store(&a[i], b_vec1);
        SIMD::store(&a[i + stride], b_vec2);
        SIMD::store(&a[i + stride * 2], b_vec3);
        SIMD::store(&a[i + stride * 3], b_vec4);
    ...
    for (size_t i = 0; i < SIZE; i++)
    {
        a[i] = b[i] + c[i] + d[i] + e[i] + f[i] - g[i] - h[i] - p[i] - q[i];
    }
#endif
}
