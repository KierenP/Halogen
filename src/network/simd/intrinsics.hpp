...
inline veci32 dpbusd_i32(const veci32& source, const vecu8& a, const veci8& b)
{
    ...
    // Multiply and accumulate - vmull produces int16x8
    ...
    // Pairwise add to get 4 int32 values from each
    int32x4_t acc_low = vpaddlq_s16(mul_low);
    int32x4_t acc_high = vpaddlq_s16(mul_high);

    // Add both results and add to source
    return vaddq_s32(source, vaddq_s32(acc_low, acc_high));
#endif
}
