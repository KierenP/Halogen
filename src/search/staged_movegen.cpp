...
//
// Each 128-bit vector holds four ExtendedMoves; the int16 score sits in the odd int16 lane of each pair. We use SSE4
// directly rather than the widest available vector: the move lists are short enough that a 128-bit window is the best
// fit, and the same code runs on every x86 build.
#if defined(USE_SSE4)
ExtendedMoveList::iterator first_max_element(ExtendedMoveList::iterator begin, ExtendedMoveList::iterator end)
{
    ...
    static_assert(offsetof(ExtendedMove, score) == 2);
    ...
    const auto floor = _mm_set1_epi16(INT16_MIN);
    // -1 (keep) over the odd score lanes, 0 (replace with floor) over the even move lanes.
    const auto score_mask = _mm_set_epi16(-1, 0, -1, 0, -1, 0, -1, 0);
    ...
    for (; i + entries <= n; i += entries)
    {
        ...
        vmax = _mm_max_epi16(vmax, _mm_blendv_epi8(floor, v, score_mask));
    }

    auto r = _mm_max_epi16(vmax, _mm_shuffle_epi32(vmax, _MM_SHUFFLE(1, 0, 3, 2)));
    r = _mm_max_epi16(r, _mm_shuffle_epi32(r, _MM_SHUFFLE(2, 3, 0, 1)));
    r = _mm_max_epi16(r, _mm_shufflelo_epi16(r, _MM_SHUFFLE(2, 3, 0, 1)));
    int16_t max_score = static_cast<int16_t>(_mm_extract_epi16(r, 0));
    for (auto it = begin + i; it != end; ++it)
    {
        max_score = std::max(max_score, it->score);
    }

    const auto target = _mm_set1_epi16(max_score);
    for (i = 0; i + entries <= n; i += entries)
    {
        const auto v = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&*(begin + i)));
        const unsigned mask = _mm_movemask_epi8(_mm_and_si128(_mm_cmpeq_epi16(v, target), score_mask));
        if (mask != 0)
        {
            return begin + i + (std::countr_zero(mask) >> 2);
        }
    }
